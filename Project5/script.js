...
//Generating the entire library of "Real" customers when page load.
function initialize() {
    basicCustomerPrice = 0;
    compCustomerPrice = 0;
    suckerCustomerPrice = 0;
    unlimitedCustomerPrice = 0;
    bestPrice = 0;

    userPrice = document.getElementById("AV");
    AV = 0;
    bestPlanCustomers = document.getElementById("BC");
    BC = 0;
    notBestPlanCustomers = document.getElementById("WC");
    WC = 0;
    basicCustomers = document.getElementById("CB");
    CB = 0;
    compCustomers = document.getElementById("CC");
    CC = 0;
    suckerCustomers = document.getElementById("CS");
    CS = 0;
    unlimitedCustomers = document.getElementById("CU");
    CU = 0;
    unlimitedBenefit = document.getElementById("BU");
    BU = 0;
    hPCNB = document.getElementById("PCNB");
    PCNB = 0;
    hPCB = document.getElementById("PCB");
    PCB = 0;
    table = document.getElementById("CustomerTable");

    numOfCustomer = document.getElementById("Total");
    Total = getRandomInteger(1000, 10000);

    for (var i = 0; i < Total; i++) {
        window['customer' + i] = new customer(generateName(), generatePhone(), generatePlan(), generateData());
        holyDATA.push(window['customer' + i]);
        //console.log(holyDATA[i]);
    }

    /*for(var y = 0; y < holyDATA.length; y++)
    {
      for(var z = 0; z < holyDATA.length; z++)
      {
        if(holyDATA[y].number==holyDATA[z].number)
        {
          if(!(y==z))
          {
    ...
    addtoTable();
    numOfCustomer.innerHTML = Total;
    basicCustomers.innerHTML = CB;
    compCustomers.innerHTML = CC;
    suckerCustomers.innerHTML = CS;
    unlimitedCustomers.innerHTML = CU;
    unlimitedBenefit.innerHTML = BU;
    bestPlanCustomers.innerHTML = BC;
    notBestPlanCustomers.innerHTML = WC;
    userPrice.innerHTML = Math.floor(AV / holyDATA.length);
    hPCNB.innerHTML = Math.floor((WC / holyDATA.length) * 100);
    hPCB.innerHTML = Math.floor((BC / holyDATA.length) * 100);
}

//Customer class
function customer(na, n, p, d, bp, cp, sp, up) {
    this.name = na;
    this.number = n;
    this.plan = p;
    this.data = d;
...
function generatePhone() {
    phoneNum = "(" + getRandomInteger(1, 9);
    //console.log(phoneNum);
    for (var y = 0; y < 2; y++) {
        phoneNum += getRandomInteger(0, 9);
    }
    phoneNum += ") ";
    for (var y = 0; y < 3; y++) {
        phoneNum += getRandomInteger(0, 9);
...
function generatePlan() {
    planChoice = getRandomInteger(1, 4);
    switch (planChoice) {
        case 1:
            return "Basic";
        case 2:
...
function generateData() {
    dataChoice = getRandomInteger(1, 40000);
    return dataChoice;
}
...
//calculate the each place price for the data
function setPlanPrice(index) {
    //console.log(index);
    holyDATA[index].bPlan = checkBasicPlan(index);
    holyDATA[index].cPlan = checkComprehensivePlan(index);
    holyDATA[index].sPlan = checkSuckerPlan(index);
    holyDATA[index].uPlan = 49.99;
}

//Check which plan the customer is in and data addition
function checkSpecificPlan(index) {
    setPlanPrice(index);

    switch (holyDATA[index].plan) {
        case "Basic":
            var temp = holyDATA[index].bPlan;
            //AV += temp;
            basicCustomerPrice += temp;
            CB++;
            break;
        case "Comprehensive":
            var temp = holyDATA[index].cPlan;
            //AV += temp;
            compCustomerPrice += temp;
            //console.log(compCustomers);
            CC++;
            break;
        case "Sucker":
            var temp = holyDATA[index].sPlan;
            //AV += temp
            suckerCustomerPrice += temp;
            CS++;
            break;
        case "Unlimited":
            var temp = holyDATA[index].uPlan;
            //AV += temp
            unlimitedCustomerPrice += temp;
            CU++;
            break;
        default:
            break;
...
//Analytical table to see if customer in best plan or not and how much they are over paying.
function checkBestPlan(index) {
    basicp = checkBasicPlan(index);
    comprehensivep = checkComprehensivePlan(index);
    suckerp = checkSuckerPlan(index);
    bestPrice = Math.min(basicp, comprehensivep, suckerp, 49.99);
    ...
    switch (holyDATA[index].plan) {
        case "Basic":
            ...
            break;
        case "Comprehensive":
            if (bestPrice < comprehensivep) {
                WC++;
                AV += (comprehensivep - bestPrice);
            }
            else {
                BC++;
            }
            break;
        case "Sucker":
            if (bestPrice < suckerp) {
                WC++;
                AV += (suckerp - bestPrice);
            }
            else {
                BC++;
            }
...
//basic plan price calculator
function checkBasicPlan(index) {
    let price = 19.99;
    if (holyDATA[index].data < 1000) {
        return price;
    }
    let extraData = 0;
    extraData = (holyDATA[index].data - 1000) * 0.1;
    return Math.round((price + extraData) * 100) / 100;
}

//Comprehensive plan price calculator
function checkComprehensivePlan(index) {
    let price = 24.99;
    if (holyDATA[index].data < 4000) {
        return price;
    }
    let extraData = 0;
    extraData = (holyDATA[index].data - 4000) * 0.25;
    return Math.round((price + extraData) * 100) / 100;
}

//Sucker plan price calculator
function checkSuckerPlan(index) {
    let price = 4.99;
    extraData = holyDATA[index].data * 0.02;
    return Math.round((price + extraData) * 100) / 100;
}

//Unlimited Plan price calculator
/*function checkUnlimitedPlan(index)
{
  let price = 49.99;
  return price;
}*/
...
function addtoTable() {
    for (var i = 0; i < holyDATA.length; i++) {
        checkSpecificPlan(i);
        checkBestPlan(i);
        //console.log(holyDATA[i].bPlan);
